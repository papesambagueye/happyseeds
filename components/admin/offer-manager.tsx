...
type Product = { id: string; name: string; description?: string | null; price: number; stock: number; image: string | null }
...
export function OfferManager({ kind }: { kind: 'promotion' | 'flash' }) {
  ...
  const openEdit = (row: Offer) => {
    ...
    setFlashForm({ ...blankFlash, productId: row.productId, productName: product?.name ?? row.productName, description: product?.description ?? '', image: product?.image ?? row.productImage ?? '', originalPrice: String(product?.price ?? row.productPrice), price: String(row.salePrice ?? row.promotionalPrice ?? ''), active: row.active === 1, startsAt: row.startsAt ? toLocal(row.startsAt) : '', endsAt: row.endsAt ? toLocal(row.endsAt) : '' })
    setOpen(true)
  }
  ...
  return (
    <AdminShell>
      ...
      <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editing ? 'Modifier' : 'Nouvelle'} {singular}</DialogTitle></DialogHeader><form onSubmit={save} className="space-y-4">
        {isPromotion && <div className="space-y-1.5"><Label>Produit du catalogue</Label><select value={form.productId} onChange={(event) => setForm({ ...form, productId: event.target.value })} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="">Choisir un produit</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} - {formatPrice(product.price, 'FCFA')}</option>)}</select></div>}
        {!isPromotion && <><div className="space-y-1.5"><Label>Nom du produit</Label><Input value={flashForm.productName} onChange={(event) => setFlashForm({ ...flashForm, productName: event.target.value })} required /></div><div className="space-y-1.5"><Label>Description</Label><Input value={flashForm.description} onChange={(event) => setFlashForm({ ...flashForm, description: event.target.value })} /></div><ImageUploadField label="Image du produit" value={flashForm.image || null} onChange={(image) => setFlashForm({ ...flashForm, image: image ?? '' })} /></>}
        {!isPromotion && <div className="space-y-1.5"><Label>Prix de vente (FCFA)</Label><Input type="number" min="1" value={flashForm.price} onChange={(event) => setFlashForm({ ...flashForm, price: event.target.value })} required /></div>}
        {isPromotion && <div className="space-y-1.5"><Label>Nouveau prix (FCFA)</Label><Input type="number" min="1" value={form.price} onChange={(event) => setForm({ ...form, price: event.target.value })} required /></div>}
